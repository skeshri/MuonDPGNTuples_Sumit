...
{
  edm::InputTag & iTag = m_config->m_inputTags["cscSegmentTag"];
  if (iTag.label() != "none") m_cscSegmentToken = collector.consumes<CSCSegmentCollection>(iTag);
...
void MuNtupleCSCSegmentFiller::fill(const edm::Event &ev, const edm::EventSetup &iSetup)
{
